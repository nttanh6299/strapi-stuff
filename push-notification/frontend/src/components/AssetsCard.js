...
const AssetsCard = (props) => {
  ...
  const deleteAssets = () => {
    StateManager?.token &&
      StateManager?.endpoints.deleteSingleAsset(
        props.data.id,
        (success, error) => {
          if (error) {
        ...
        <div className="bg-b ">
          ...
          <div className="flex flex-wrap py-2 px-2">
            <div className="w-6/12 py-1">
              ...
              <p className="w-9/12 break-words">
                {props?.data?.attributes?.model_number}
              </p>
            </div>
        ...
        <div className="card-overlay flex items-center justify-center">
          <div className="flex md:-mx-2 -mx-4">
            <div className="w-6/12 md:px-2 px-4">
              <Link
                to={"/asset/edit/" + props?.data?.id}
                className="inline-block border-radius-50p bg-CA9140 md:p-4 p-10"
              >
                &#10003;
              </Link>
            </div>
