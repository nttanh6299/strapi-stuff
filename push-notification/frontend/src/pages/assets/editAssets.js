...
const EditAssets = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setloading(true);
    StateManager?.endpoints.editAssets(id, state, (success, error) => {
      if (error) {
  ...
  const getAsset = useCallback(() => {
    ...
    return StateManager?.endpoints.getSingleAsset(id, (success, error) => {
      ...
      if (success) {
        ...
        setstate(() => ({
          name: success.data?.data?.attributes?.name,
          description: success.data?.data?.attributes?.description,
          model_number: success.data?.data?.attributes?.model_number,
          validity_period:
            success.data?.data?.attributes?.validity_period,
          category: success.data?.data?.attributes?.category,
          is_available: success.data?.data?.attributes?.is_available,
          logs: [],
          is_expired: success.data?.data?.attributes?.is_expired,
        }));
      }
      setloading(false);
    });
