...
const AssetsPage = (props) => {
  ...
  useEffect(() => {
    if (!StateManager?.token || isFetchedRef.current) return
    StateManager.endpoints.getAssets();
    isFetchedRef.current = true
  }, [StateManager?.token, StateManager.endpoints]);
  return (
    <div>
      <div className="p-4 md:px-8 md:py-0">
        <div className="flex justify-center items-center md:block">
          ...
          <div className="md:hidden pb-10">
            <div className="px-4 py-4">
              ...
              <button
                onClick={StateManager.endpoints.logout}
                className="text-capitalize c-CA9140"
              >
                LOGOUT{" "}
              </button>
            </div>
        ...
        <div className="mt-8 md:flex md:flex-wrap my-2 md:-my-4">
          {StateManager?.assets?.allAssets ? (
            StateManager.assets.allAssets.map((curr, idx) => {
              return (
