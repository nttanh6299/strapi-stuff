...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    setloading(true);
    !StateManager?.token &&
      StateManager?.endpoints.login(state, async (success, error) => {
        if (error) {
        ...
        <div className="md:w-6/12">
          <form className="px-6 py-10" onSubmit={handleSubmit}>
            ...
            <div className="w-full mb-6">
              <input
                className="inline-block w-full p-6 c-black border"
            ...
            {error ? (
              <p style={{ color: "red" }} className="mt-5 text-center">
                {error}
              </p>
            ) : null}

            <p className="mt-16 text-center">
              to Signup, click <Link to="/signup">here</Link>
            </p>
          </form>
        </div>
      </div>
